...
class renWin():
	...
	def renPreProcess(self):
		...
		clrEdgy = self.parmDic["clrEdgy"]
		clrEdgyDist = self.parmDic["clrEdgyDist"]
		...
		diskIntens = float(self.parmDic["diskIntens"])
		...
		for i in range(nSpokes):
			...
			"uniform float ndcEdgy" [ """ + str(clrEdgy) + """ ]
			"uniform float ndcEdgyDist" [ """ + str(clrEdgyDist) + """ ]
			"uniform color fadeClr" [ """ + vecToString(fadeClr) + """ ]
			TransformBegin
			...
			for j in range(nStuds):
				...
				TransformBegin
					...
					Torus """ + str(rad) + """ """ + str(minRad) + """ 60 90 360 
				        Color """ + str(kk*clr[0])  + " " + str(kk*clr[1]) + " " + str(kk*clr[2]) + """
                                        AttributeBegin
                                            Surface "lDisk"
                                                "uniform float ndcFadeEdge" [ """ + str(edgeFade) + """ ]
                                                "uniform float ndcEdgy" [ """ + str(clrEdgy) + """ ]
                                                "uniform float ndcEdgyDist" [ """ + str(clrEdgyDist) + """ ]
                                            Opacity """ + str(op) + " " + str(op) + " " + str(op) + """
                                            #Disk 0 """ + str(rad) + """ 360
                                        AttributeEnd
				TransformEnd
				"""
...
"""
		...
		rib += """
			...
			"uniform float diskIntens" [ """ + str(diskIntens) + """ ]
                        Opacity 1 1 1
		...
		if self.parmDic["compText"] == 1:
			for img,ofs,sc,k in [(logoImg, logoOfs, logoSc, 1), (titleImg, titleOfs, titleSc, 1), (infoImg, infoOfs, infoSc, infoK)]:
			...
			#print
				...
				#scFl = .2
				scRel = str(scFl * float(self.res[1])/1336)
				print "----- scRel", scRel
				#scRel = ".2"
				#scRel = str(scFl)
				cmd = "convert " + img + " -resize " + scRel + "% -fill '" + rgb_to_hex(clrText[0], clrText[1], clrText[2]) + "' -colorize 100% " + textImgClr + "; "
				cmd += "convert " + img + " -resize " + scRel + "% " + textImgSc + "; "
				num =  str(float(k)*100 - 100)
				#cmd += "\nconvert -brightness-contrast " + num + "x0 " + textImgClr + " " + textImgClr + "; "
				cmd += "\ncomposite " + textImgSc + " -compose Multiply " + textImgClr + " " + tempImg
				exeCmd(cmd)
				print
	...
	def renPostProcess(self):
		...
		rib += """
			...
			"uniform float rx" [ """ + str(self.res[0]) + """ ]
			"uniform float ry" [ """ + str(self.res[1]) + """ ]
			ShadingRate .05
			WorldBegin
			WorldEnd
			"""
	...
	def ren(self):
		...
		self.res = (rx*resScale, ry*resScale)
	...
	def __init__(self):
		...
		self.photo = PhotoImage(file=gifRenDisplay);
